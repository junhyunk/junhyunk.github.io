...
PowermMock의 사용법은 Mockito와 비슷합니다. Util 함수에 대한 수행만 테스트할 것이고, jUnit 테스트에 사용하기 위해 `@Runwith`에는 `PowerMockRunner` 클래스를 적용합니다. 테스트 메소드에 앞서, `@PrepareForTest`에는 어떤 클래스(스태틱 메소드가 포함된)를 mocking할 것인지 선언하고, `PowerMockito.mockStatic` 메소드를 통해 해당 클래스를 mocking합니다. 그리고 Mockito와 비슷하게 when/thenReturn 함수를 통해 동작을 정의합니다.
